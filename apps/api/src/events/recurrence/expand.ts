...
export function expandRecurringMaster(
  ...
  try {
    if (ev.allDay) {
      ...
      // Optional debug logging for CI troubleshooting. Set DEBUG_RECURRENCE=1
      // in the environment (and keep NODE_ENV=test) to dump the naive window
      // and the rule occurrences generated by rrule. This is intentionally
      // quiet by default so test output stays clean.
      if (
        process.env.NODE_ENV === 'test' &&
        process.env.DEBUG_RECURRENCE === '1'
      ) {
        try {
